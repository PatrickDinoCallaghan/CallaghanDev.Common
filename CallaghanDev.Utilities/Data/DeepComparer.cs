...
namespace CallaghanDev.Utilities
{
    ...
    public class DeepComparer
    {
        public bool AreObjectsEqual(object obj1, object obj2)
        {
            ...
            {
                Console.WriteLine($"Type mismatch: {obj1.GetType()} vs {obj2.GetType()}");
                return false;
            }
            ...
            foreach (var property in properties)
            {
                if (property.PropertyType.IsPrimitive || property.PropertyType == typeof(string))
                {
                    ...
                    {
                        Console.WriteLine($"Property mismatch in {property.Name}: {value1} vs {value2}");
                        return false;
                    }
                }
                else
        ...
        private bool CompareEnumerables(IEnumerable enum1, IEnumerable enum2)
        {
            ...
            {
                Console.WriteLine("Enumerable length mismatch.");
                return false;
            }
        ...
        private bool CompareDictionaries(IDictionary dict1, IDictionary dict2)
        {
            ...
            {
                Console.WriteLine("Dictionary count mismatch.");
                return false;
            }
            ...
            foreach (DictionaryEntry entry in dict1)
            {
                ...
                {
                    Console.WriteLine($"Dictionary key missing: {entry.Key}");
                    return false;
                }
                ...
                {
                    Console.WriteLine($"Dictionary value mismatch for key {entry.Key}: {value1} vs {value2}");
                    return false;
                }
            }
