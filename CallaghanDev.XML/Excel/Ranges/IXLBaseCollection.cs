...
namespace CallaghanDev.XML.Excel
{
    public interface IXLBaseCollection<TSingle, TMultiple> : IEnumerable<TSingle>
    {
        ...
        /// <para>If the object is an IEnumerable CallaghanDev.XML will copy the collection's data into a table starting from each cell.</para>
        /// <para>If the object is a range CallaghanDev.XML will copy the range starting from each cell.</para>
        /// <para>Setting the value to an object (not IEnumerable/range) will call the object's ToString() method.</para>
        /// <para>CallaghanDev.XML will try to translate it to the corresponding type, if it can't then the value will be left as a string.</para>
        /// </summary>
        /// <value>
